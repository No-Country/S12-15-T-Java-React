...
const InputPasswordsConfirm = ({ passwords, setPasswords }) => {
	...
	const togglePasswordVisibility = (inputPassword) => {
		setShowPassword((prevShowPasswords) => ({
			...prevShowPasswords,
			[inputPassword]: !prevShowPasswords[inputPassword],
		}));
	};

	const handleChange = (e, inputPassword) => {
		...
		switch (inputPassword) {
			...
			case 'confirmPassword':
				if (value === passwords.password) {
					...
					setError((prevErrors) => ({
						...prevErrors,
						confirmPassword: 'Completa este campo',
					}));
				} else {
					setError((prevErrors) => ({
						...prevErrors,
		...
		<>
			<div className={stylesRegister.boxInput}>
				<input
					...
					className={`${stylesRegister.input} ${
						error.password && stylesRegister.error
					}`}
					required
				/>
				<svg
			...
			{error.password && (
				<span className={stylesRegister.errorMessage}>{error.password}</span>
			)}
			<div className={stylesRegister.boxInput}>
				<input
					...
					className={`${stylesRegister.input} ${
						error.confirmPassword && stylesRegister.error
					}`}
					required
				/>
				<svg
			...
			{error.confirmPassword && (
				<span className={stylesRegister.errorMessage}>
					{error.confirmPassword}
				</span>
			)}
		</>
...
export default InputPasswordsConfirm;
