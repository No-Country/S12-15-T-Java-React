...
import ChannelGiphyPicker from './ChannelGiphyPicker';

const ChannelWriteMessage = ({ handleSendMessage }) => {
	const [newMessage, setNewMessage] = useState('');
	const [isEmojiPickerVisible, setIsEmojiPickerVisible] = useState(false);
	const [isGiphyPickerVisible, setIsGiphyPickerVisible] = useState(false);

	const toggleEmojiPickerVisibility = () => {
		setIsEmojiPickerVisible(!isEmojiPickerVisible);
	};

	const toggleGiphyPickerVisibility = () => {
		setIsGiphyPickerVisible(!isEmojiPickerVisible);
	};
	...
	const handleGifClick = (gifUrl) => {
		if (gifUrl) {
			handleSendMessage(`[giphy]${gifUrl}[/giphy]`);
			setIsGiphyPickerVisible(!isEmojiPickerVisible);
		}
	};
			...
			<div className={styles.menu}>
				<div>
					...
					<svg
						...
						viewBox="0 0 20 20"
						fill="none"
						onClick={toggleGiphyPickerVisibility}
					>
						<path
				...
				{isGiphyPickerVisible && (
					<div className={styles.pickerContainer}>
						<ChannelGiphyPicker
							onGifClick={handleGifClick}
							setIsGiphyPickerVisible={setIsGiphyPickerVisible}
						/>
					</div>
				)}
				<button type="submit">
